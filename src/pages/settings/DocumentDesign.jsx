...
import {
  ...
  Palette, Type, Layout, Sparkles, CheckCircle2, Trash2, Star,
  RotateCcw, Eye
} from "lucide-react";
...
const ZOOM_LEVELS = [0.5, 0.65, 0.75, 1, 1.25];
const ZOOM_LABELS = ["50%", "65%", "75%", "100%", "125%"];
// A4 intrinsic dimensions at 96dpi
const DOC_W = 794;
const DOC_H = 1123;
...
export default function DocumentDesign() {
  ...
  const [manualZoom, setManualZoom]     = useState(3); // index into ZOOM_LEVELS
  ...
  const saveTheme = () => {
    ...
    const next = [...savedThemes.filter(t => t.name !== savingThemeName), { name: savingThemeName.trim(), design: { ...design } }];
    setSavedThemes(next);
    base44.auth.updateMe({ doc_design_saved_themes: JSON.stringify(next) });
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-white border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-0.5 bg-muted rounded-lg p-0.5">
            <button
              onClick={() => { setUseAutoFit(false); setManualZoom(z => Math.max(0, z - 1)); }}
              disabled={!useAutoFit && manualZoom === 0}
              className="p-1.5 rounded-md hover:bg-white transition-colors text-muted-foreground hover:text-foreground disabled:opacity-30">
            ...
            <button
              onClick={() => { setUseAutoFit(false); setManualZoom(z => Math.min(ZOOM_LEVELS.length - 1, z + 1)); }}
        ...
        <div className="flex items-center gap-2">
          {isDirty && (
            <button onClick={() => { setDesign({ ...savedDesign }); setActivePresetId(null); }}
              className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground border border-border rounded-lg px-3 py-1.5 transition-colors">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        ...
        {/* Center: Live Preview — no scroll, always fits */}
        <div
          ref={containerRef}
          className="flex-1 min-w-0 bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center overflow-hidden"
        >
          {/* Outer box sized to scaled document */}
          <div style={{
            ...
            {/* Inner document at full size, scaled down via transform */}
            <div style={{
              width: previewW,
              height: DOC_H,
              ...
              <DocumentPreview
                ...
                sym="₦"
                docType="invoice"
                managerSig={previewDoc.manager_signature}
                customerSig=""
        ...
        {/* Right: Presets & My Themes */}
        <div className="w-60 shrink-0 bg-white border-l border-border overflow-y-auto flex flex-col">
            ...
            <div className="space-y-1">
              {PRESETS.map(p => (
                <button key={p.id} onClick={() => applyPreset(p)}
                  className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-xl text-left transition-all border ${activePresetId === p.id ? "border-indigo-300 bg-indigo-50" : "border-transparent hover:border-border hover:bg-muted/40"}`}>
          ...
          <div className="px-4 pt-3 pb-4 flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setShowSaveInput(s => !s)}
            ...
            {/* AI Suggestions */}
            <div className="mt-5 bg-gradient-to-br from-indigo-50 to-purple-50 border border-indigo-100 rounded-2xl p-3">
