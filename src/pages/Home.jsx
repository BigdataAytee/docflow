...
const DOC_TYPES = [
{ type: "invoice", label: "Invoice", description: "Request payment from a customer", icon: FileText, gradient: "linear-gradient(135deg, #3b82f6 0%, #1d4ed8 100%)", glow: "rgba(59,130,246,0.25)", path: "/documents?type=invoice", newPath: "/documents/new?type=invoice" },
{ type: "quotation", label: "Price Quote", description: "Show a customer what something costs", icon: FileCheck, gradient: "linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%)", glow: "rgba(139,92,246,0.25)", path: "/documents?type=quotation", newPath: "/documents/new?type=quotation" },
{ type: "receipt", label: "Receipt", description: "Confirm you received a payment", icon: Receipt, gradient: "linear-gradient(135deg, #10b981 0%, #047857 100%)", glow: "rgba(16,185,129,0.25)", path: "/documents?type=receipt", newPath: "/documents/new?type=receipt" },
{ type: "waybill", label: "Delivery Note", description: "Record what is being delivered", icon: Truck, gradient: "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)", glow: "rgba(245,158,11,0.25)", path: "/documents?type=waybill", newPath: "/documents/new?type=waybill" },
{ type: "mail", label: "My Emails", description: "View emails sent to customers", icon: Mail, gradient: "linear-gradient(135deg, #ef4444 0%, #b91c1c 100%)", glow: "rgba(239,68,68,0.25)", path: "/mail", newPath: null }];
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-white/70 mt-2 text-base">What would you like to do today?</p>
          ...
          <div className="flex flex-wrap gap-3 shrink-0">
            <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-3 text-center min-w-[80px]">
              <p className="text-2xl font-bold text-white">{docs.length}</p>
              <p className="text-white/60 text-sm mt-0.5">Documents</p>
            </div>
            <div className="bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl px-4 py-3 text-center min-w-[80px]">
              <p className="text-2xl font-bold text-emerald-300">{docs.filter((d) => d.status === "paid" || d.status === "delivered").length}</p>
              <p className="text-white/60 text-sm mt-0.5">Completed</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="🔍  Find a document — type a customer name or document number"
          className="w-full h-14 pl-11 pr-11 rounded-2xl border border-border bg-card text-base focus:outline-none focus:ring-2 focus:ring-ring shadow-sm placeholder:text-muted-foreground transition-shadow focus:shadow-md" />
      ...
      <>
          <div>
            <h2 className="font-bold text-lg text-foreground mb-4">What do you need to create?</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
              ...
              return (
                ...
                style={{
                  ...
                  transition: "all 0.25s ease"
                    ...
                    <div className="relative z-10 p-4 md:p-6 flex flex-col gap-3 min-h-[168px] md:min-h-[190px]">
                      <div className="flex items-start justify-between">
                        <div className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-white/20 backdrop-blur-sm flex items-center justify-center shadow-inner">
                          <Icon className="h-6 w-6 md:h-7 md:w-7 text-white" />
                        </div>
                        {count !== null && count > 0 &&
                      <span className="text-sm font-bold px-2.5 py-1 rounded-full bg-white/25 text-white backdrop-blur-sm">{count} saved</span>
                      }
                      </div>
                      <div className="flex-1">
                        <h3 className="font-bold text-white text-lg md:text-xl leading-tight">{label}</h3>
                        <p className="text-white/80 text-sm mt-1 leading-relaxed">{description}</p>
                      </div>
                      {newPath ?
                    <button onClick={(e) => {e.stopPropagation();navigate(newPath);}}
                    className="flex items-center gap-2 text-sm font-bold text-white bg-white/25 hover:bg-white/40 backdrop-blur-sm rounded-xl px-4 py-2.5 w-fit transition-colors">
              ...
              style={{ background: "linear-gradient(90deg, transparent, rgba(99,102,241,0.6), rgba(16,185,129,0.4), transparent)" }} />
                ...
                <div className="relative z-10 p-5 md:p-6 flex items-center gap-5">
                  ...
                  {/* Text */}
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl font-black text-white leading-tight">Your Business Summary</h3>
                    <p className="text-sm mt-1 hidden sm:block" style={{ color: "rgba(255,255,255,0.55)" }}>
                      See how much you've earned, who your best customers are, and more
                    </p>
                  </div>
        ...
        <div>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-bold text-lg text-foreground">Recently Created</h2>
                </div>
            ...
            className="text-sm text-primary font-semibold flex items-center gap-1.5 hover:underline">
              ...
              return (
                <div key={doc.id} onClick={() => navigate(`/documents/${doc.id}`)}
                 className={`flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-muted/40 transition-colors group ${idx < recentDocs.length - 1 ? "border-b border-border" : ""}`}>
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm" style={{ background: meta.gradient }}>
                        <Icon className="h-5 w-5 text-white" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-base truncate">{doc.number || "—"}</p>
                        <p className="text-sm text-muted-foreground truncate">{doc.customer_name || "No customer"}</p>
                      </div>
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="text-right hidden sm:block">
                          <span className="text-sm font-semibold px-3 py-1 rounded-full capitalize block" style={{ background: statusColor + "1a", color: statusColor }}>
                            {doc.status?.replace(/_/g, " ")}
                          </span>
