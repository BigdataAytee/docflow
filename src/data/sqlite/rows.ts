...
export const toCompany = (row: SqlRow): Company => ({
  ...
  ...optional('documentNote', text(row['document_note'])),
...
export const companyColumns = (company: Company): Record<string, SqlValue> => ({
  ...
  document_note: company.documentNote ?? null,
...
export const toDocument = (row: SqlRow): DocumentRecord => ({
  ...
  ...optional('signerSignatureAssetId', text(row['signer_signature_asset_id'])),
  ...optional('billsBalanceOfId', text(row['bills_balance_of_id'])),
  ...optional('billedTotalMinor', int(row['billed_total_minor'])),
  ...optional(
    'deductions',
    parseJson<{ paidAt: string; amountMinor: number }[] | undefined>(row['deductions'], undefined),
  ),
  ...optional('balanceAfterMinor', int(row['balance_after_minor'])),
  ...optional('invoiceTotalMinor', int(row['invoice_total_minor'])),
  ...optional('paidBeforeMinor', int(row['paid_before_minor'])),
...
export const documentColumns = (document: DocumentRecord): Record<string, SqlValue> => ({
  ...
  signer_signature_asset_id: document.signerSignatureAssetId ?? null,
  bills_balance_of_id: document.billsBalanceOfId ?? null,
  billed_total_minor: document.billedTotalMinor ?? null,
  deductions: document.deductions === undefined ? null : json(document.deductions),
  balance_after_minor: document.balanceAfterMinor ?? null,
  invoice_total_minor: document.invoiceTotalMinor ?? null,
  paid_before_minor: document.paidBeforeMinor ?? null,
