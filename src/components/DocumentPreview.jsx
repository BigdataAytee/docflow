    ...
    <div style={{ width: 760, background: "#fff", border: "1px solid #e2e8f0", borderRadius: 8, overflow: "hidden", fontFamily: "'Helvetica Neue', Arial, sans-serif", minHeight: 1040 }}>
      ...
      <div style={{ borderBottom: "2px solid #1e293b", padding: "20px 32px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 14, color: "#94a3b8", marginTop: 4, fontFamily: "monospace" }}>{form.number || "—"}</div>
          {!isLetter && (
            <div style={{ marginTop: 8, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 6, padding: "4px 12px", display: "inline-block" }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 900, color: "#1e293b" }}>{sym}{fmt(calcs?.total || 0)}</div>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Dates strip */}
      {!isLetter && (
        <div style={{ display: "flex", gap: 32, padding: "12px 32px", background: "#f8fafc", borderBottom: "1px solid #e2e8f0", borderTop: "1px solid #e2e8f0", fontSize: 11 }}>
        ...
        <>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${cfg.accent}`, background: "#f8fafc" }}>
                <th style={{ textAlign: "left", padding: "10px 32px", color: "#475569", fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: 0.5, borderBottom: "2px solid #1e293b" }}>Description</th>
                    <th style={{ textAlign: "right", padding: "10px 16px", color: "#475569", fontWeight: 700, fontSize: 10, textTransform: "uppercase", borderBottom: "2px solid #1e293b" }}>Qty</th>
                    <th style={{ textAlign: "right", padding: "10px 16px", color: "#475569", fontWeight: 700, fontSize: 10, textTransform: "uppercase", borderBottom: "2px solid #1e293b" }}>Rate</th>
                    <th style={{ textAlign: "right", padding: "10px 32px 10px 16px", color: "#475569", fontWeight: 700, fontSize: 10, textTransform: "uppercase", borderBottom: "2px solid #1e293b" }}>Amount</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", padding: "16px 32px 24px" }}>
            <div style={{ width: 260, fontSize: 12 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 0 8px", borderTop: "2px solid #1e293b", marginTop: 6, fontWeight: 800, fontSize: 15, color: "#1e293b" }}>
                <span>Total</span><span style={{ color: "#1e293b" }}>{sym}{fmt(calcs?.total)}</span>
              </div>
            </div>
          </div>
        </>
