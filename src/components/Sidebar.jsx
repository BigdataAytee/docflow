...
const navItems = [
  ...
  { label: "Help & Guide",     icon: HelpCircle, path: "/help" },
];
...
export default function Sidebar({ onClose }) {
  ...
  return (
    <aside className="h-full w-64 flex flex-col" style={{ background: "linear-gradient(180deg, hsl(224,30%,10%) 0%, hsl(228,28%,13%) 100%)" }}>
      ...
      {/* Nav label */}
      <p className="px-5 text-xs font-bold uppercase tracking-widest text-white/30 mb-2">Menu</p>
        ...
        {navItems.map((item, idx) => {
          ...
          return (
            <Link
              ...
              to={item.path}
              onClick={() => onClose && onClose()}
              className={`flex items-center gap-3 px-3 py-3 rounded-xl text-base font-semibold transition-all group ${
                active
                  ? "bg-white/10 text-white shadow-sm"
            ...
            >
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-all ${
                ...
                <item.icon className={`h-4 w-4 transition-colors ${ active ? "text-white" : "text-white/60 group-hover:text-white/80" }`} />
              </div>
              {item.label}
      ...
      <div className="p-3">
        <button
          ...
          className="flex items-center gap-3 px-3 py-3 rounded-xl text-base font-semibold text-white/50 hover:text-white hover:bg-white/[0.06] transition-all w-full group"
        >
          <div className="w-8 h-8 rounded-lg bg-white/5 group-hover:bg-white/10 flex items-center justify-center shrink-0 transition-all">
