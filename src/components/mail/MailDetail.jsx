...
export default function MailDetail({ mail, onClose, onStar, onDelete, onReply }) {
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden">
      {/* Header toolbar */}
      <div className="flex items-center gap-1 px-4 py-2 border-b border-[#e0e0e0] shrink-0">
        <button onClick={onClose} className="p-2 hover:bg-[#e8eaed] rounded-full -ml-2 mr-2 text-[#444746]">
          <ArrowLeft className="h-5 w-5" />
        </button>
        <button className="p-2 hover:bg-[#e8eaed] rounded-full text-[#444746]" title="Archive">
          <ArrowLeft className="h-4 w-4 rotate-90" />
        </button>
        <button onClick={() => onDelete?.(mail)} className="p-2 hover:bg-[#e8eaed] rounded-full text-[#444746]" title="Delete">
          <Trash2 className="h-4 w-4" />
        </button>
        <button className="p-2 hover:bg-[#e8eaed] rounded-full text-[#444746]" title="More">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-4">
        ...
        <div className="border border-[#e0e0e0] rounded-2xl overflow-hidden">
          {/* Email header */}
          <div className="flex items-start gap-3 px-6 py-4 border-b border-[#e0e0e0]/60">
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className="text-xs text-[#5f6368] hover:text-[#202124] flex items-center gap-1"
                  >
                    {showDetails ? "Hide details" : `to ${mail.to_email}`}
                  </button>
                </div>
                <span className="text-xs text-[#5f6368] shrink-0">{date}</span>
              </div>
              {showDetails && (
            ...
            <div className="flex items-center gap-1 shrink-0">
              <button className="p-1.5 hover:bg-[#e8eaed] rounded-full text-[#5f6368]" title="Reply">
                <Reply className="h-4 w-4" />
              </button>
              <button className="p-1.5 hover:bg-[#e8eaed] rounded-full text-[#5f6368]" title="More">
                <MoreVertical className="h-4 w-4" />
              </button>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3 mt-6 flex-wrap">
          ...
          <button className="flex items-center gap-2 px-5 py-2 border border-[#dadce0] rounded-full text-sm text-[#444746] hover:bg-[#f6f8fc] transition-colors">
