...
export default function Settings() {
  ...
  const save = async () => {
    ...
    for (let i = 0; i < docs.length; i += chunkSize) {
      ...
      await Promise.all(chunk.map(doc => base44.entities.Document.update(doc.id, companyPatch)));
      if (i + chunkSize < docs.length) await new Promise(r => setTimeout(r, 300));
    }
          ...
          <Section title="Financial Defaults" emoji="💰">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <CurrencySelect value={form.default_currency} onValueChange={v => update("default_currency", v)} className="mt-1" />
              </div>
              <div>
